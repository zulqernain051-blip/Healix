...
export default function AdminLayout() {
  ...
  if (isLargeScreen) {
    const navItems = [
      ...
      { name: 'Nurse Verifications', path: '/admin/nurses', icon: '👩‍⚕️' },
      { name: 'Doctor Verifications', path: '/admin/doctors', icon: '👨‍⚕️' },
      { name: 'System Logs & Config', path: '/admin/config', icon: '⚙️' },
    ];
      ...
      <View style={styles.webContainer}>
        <View style={styles.sidebar}>
          <View style={styles.sidebarHeader}>
            ...
            <Text style={styles.brandSub}>Admin Ops Portal</Text>
          </View>

          <View style={styles.sidebarProfile}>
            <Avatar.Icon size={40} icon="shield-account" color="#00E676" style={styles.avatarBg} />
            <View style={styles.profileTextWrap}>
              <Text style={styles.sidebarProfileName} numberOfLines={1}>{user?.fullName ?? 'System Admin'}</Text>
              <Text style={styles.sidebarProfileRole}>Administrator</Text>
            </View>
          </View>

          <View style={styles.navMenu}>
            {navItems.map((item: any) => {
              const isActive = currentActive.startsWith(item.path) || (item.path === '/admin' && (currentActive === '/' || currentActive === '/(admin)'));
              return (
                <TouchableOpacity
                  key={item.path}
  ...
  // Admin Mobile Bottom Bar: 5 Tabs (Dashboard, Users, Nurses, Doctors, Config)
  return (
    <Tabs
      screenOptions={{
        headerShown: false,
    ...
    >
      ...
      <Tabs.Screen
        ...
        options={{
          title: 'Nurses',
          tabBarIcon: ({ color, focused }) => (
            <Text style={{ fontSize: 20, color }}>{focused ? '👩‍⚕️' : '👩‍⚕️'}</Text>
          ),
        }}
      />
      <Tabs.Screen
